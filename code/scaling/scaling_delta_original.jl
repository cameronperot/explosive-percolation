...
function simulate_parallel_seeds(
	...
	Threads.@threads for i in 1:n_sims
		rows[i] = row(evolution_method(graph_type(system_size, seed=seeds[i]), n_steps))
	end
	t₁ = now()
...
# simulate_system_sizes(Network, stochastic_edge_acceptance!, 2 .^ [10, 11], 10, 0.8, 10, "/tmp/test.csv")
